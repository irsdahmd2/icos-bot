...
def extract_knowledge_units(product_text: str, tier: str, product_name: str = "") -> list:
    ...
    # 2026-09-22 REAL BUG FOUND IN PRODUCTION: this used to attach the FULL
    # document-wide term list to EVERY single KU. Several of your product's
    # real section names (e.g. "Weekly Reset", "Recovery Mode", "Friction
    # Log") are also ordinary English phrases, so a post about a completely
    # different idea kept getting blocked for coincidentally using one of
    # them — and after 3 failed rewrites, the KU was being marked exhausted
    # and its content thrown away for a false alarm, not a real leak.
    # Each KU's protected list is now only the terms that ACTUALLY appear in
    # THAT KU's own source excerpt — a real leak is still always caught,
    # because the excerpt is exactly what the post is built from.
    master_terms = _clean_protected_terms(all_terms)
    out = []
    for ku in all_kus[:max_ku]:
        own = ku.pop("_own_terms", [])
        excerpt_lower = (ku.get("source_excerpt") or "").lower()
        scoped_master = [t for t in master_terms if t.lower() in excerpt_lower]
        terms = _clean_protected_terms(scoped_master + list(own))
